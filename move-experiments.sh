...
if [ "${TYPE}" == "export" ] ; then
    ...
    mkdir -p $WORK_DIR
    mkdir -p ${WORK_DIR}/${SOURCE_PROJECT_NAME}
    ...
    echo "$S_PROJECT_ID" > ${WORK_DIR}/${SOURCE_PROJECT_NAME}/id
    ...
    for exp in $EXPS ; do
        ...
        mkdir -p ${WORK_DIR}/${SOURCE_PROJECT_NAME}/${exp_name}
        echo $exp_id > ${WORK_DIR}/${SOURCE_PROJECT_NAME}/${exp_name}/id
        echo $exp > ${WORK_DIR}/${SOURCE_PROJECT_NAME}/${exp_name}/def
        ...
        for run_id in $RUN_IDS ; do
            ...
            echo $run_details > ${WORK_DIR}/${SOURCE_PROJECT_NAME}/${exp_name}/run-${run_id}.json
        done
...
### IMPORT
    ...
    mkdir -p $WORK_DIR
    mkdir -p ${WORK_DIR}/${TARGET_PROJECT_NAME}
    ...
    echo "$T_PROJECT_ID" > ${WORK_DIR}/${TARGET_PROJECT_NAME}/id
    ...
    find ${WORK_DIR}/${SOURCE_PROJECT_NAME}/* -type d -print0 | while IFS= read -r -d $'\0' exp_dir ; do

        exp_id=$( cat ${exp_dir}/id)
        export exp_name=$( basename $exp_dir )
        echo "Treating ${exp_name} with id: $exp_id"

        mkdir -p ${WORK_DIR}/${TARGET_PROJECT_NAME}/${exp_name}
        cp ${exp_dir}/def ${WORK_DIR}/${TARGET_PROJECT_NAME}/${exp_name}/
        ...
        echo $exp_id_target > ${WORK_DIR}/${TARGET_PROJECT_NAME}/${exp_name}/id
        ...
        find ${exp_dir}/* -type f -name "run*" -print0 | while IFS= read -r -d $'\0' run_file ; do
            ...
            cp $run_file ${WORK_DIR}/${TARGET_PROJECT_NAME}/${exp_name}/
            ...
            sed -i -e "s/${exp_id}/${exp_id_target}/" ${WORK_DIR}/${TARGET_PROJECT_NAME}/${exp_name}/${run_basename}
            sed -i -e "s/${run_id}/${run_id_target}/" ${WORK_DIR}/${TARGET_PROJECT_NAME}/${exp_name}/${run_basename}
            cp ${WORK_DIR}/${TARGET_PROJECT_NAME}/${exp_name}/${run_basename} run_full_def.json
